...
function Invoke-NetworkIsolation {
    ...
    begin {
        ...
        $firewallisolationrule = {
            ...
            New-NetFirewallRule -DisplayName "Block Inbound connections" -Direction Inbound -Action Block -LocalPort 0-3388, 3390-5984, 5987-65535 -Protocol TCP
        ...
        $containmentstatus = {
            ...
            write-host "[*][$(Get-TimeStamp)]["$env:COMPUTERNAME"] Attempting to add local windows firewall rule 'Block Outbound connections'. Nothing out is allowed" -ForegroundColor Yellow
            write-host "[*][$(Get-TimeStamp)]["$env:COMPUTERNAME"] Attempting to add local windows firewall rule 'Block Inbound connections'. Just RDP and WinRM are allowed" -ForegroundColor Yellow 
            ...
            if ($CheckingRulesAdded.Count -eq 2) {
                write-host "[+][$(Get-TimeStamp)]["$env:COMPUTERNAME"] Firewall rules successfully added" -ForegroundColor Green
                Write-Host "[+][$(Get-TimeStamp)]["$env:COMPUTERNAME"] Rules identifiers: " $CheckingRulesAdded.Name -ForegroundColor Green
                Write-Host "[*][$(Get-TimeStamp)]["$env:COMPUTERNAME"] Warning: If you are running this script to a remote computer do not close this Powershell Console you might lose the last working session" -ForegroundColor Yellow
            }
            else {
                write-host "[-][$(Get-TimeStamp)] Firewall rules could not be found, something went wrong" -ForegroundColor Red
                exit
            }   
        }
...
function Invoke-NetworkRelease {
    ...
    begin {
        ...
        $containmentstatus = {
            function Get-TimeStamp { get-date -Format "MM/dd/yyyy HH:mm:ss K" }
            write-host "[*][$(Get-TimeStamp)]["$env:COMPUTERNAME"] Removing local windows firewall rule 'Block Outbound connections'" -ForegroundColor Yellow
            write-host "[*][$(Get-TimeStamp)]["$env:COMPUTERNAME"] Removing to remove local windows firewall rule 'Block Inbound connections'" -ForegroundColor Yellow
            $CheckingRulesAdded = Get-NetFirewallRule | Where-Object { $_.DisplayName -match '(Block Outbound connections)|(Block Inbound connections)' }
            if ($CheckingRulesAdded.Count -eq 2) {
                write-host "[-][$(Get-TimeStamp)]["$env:COMPUTERNAME"]  Firewall could not be removed, please check your permissions and connectivity" -ForegroundColor Red
                Write-Host "Rules identifiers: " $CheckingRulesAdded.Name -ForegroundColor Red
                exit
            }
            else {
                write-host "[-][$(Get-TimeStamp)]["$env:COMPUTERNAME"]  Firewall rules successfully removed " -ForegroundColor Green
            }   
        }
        $statusparameters = @{scriptblock = $containmentstatus }
